...
Az általunk vizsgált modell eltér a megszokottól abban, hogy a sejtek ábrázolására voronoi hálózatokat használ. Térbeli játékok esetén általában szabályos pontrácsokkal (??) vagy skála-független hálózatokkal dolgoznak. Míg az előbbi figyelmen kívül hagyja a kapcsolatok sokféleségét, az utóbbi nem megfelelő, ha az egyedek egy síkban helyezkednek el \cite{archetti2016cooperation}. (?? kérdéses rész)

Legyen egy \textit{X} metrikus térben \textit{d} a távolságfüggvény, \textit{K} indexhalmaz, \(\{P_k\}_{k \in K}\) pedig \textit{X} nemüres résztereinek (subset?? részhalmaz? site) listája. A \(P_k\)-hoz tartozó \(R_k\) voronoi cella a \(P_k\)-hoz legközelebbi pontok halmaza: 
\begin{equation}
R_k = \{x \in K | d(x,P_k) \leq d(x,P_j), \forall j \neq k\}
\end{equation}
